...
def scrape_page_html(context: BrowserContext, source: SourceConfig, output_dir: Path) -> ScrapedArtifact:
    """
    Visit `source.url` and extract only business-relevant fund information.
    """
    ...
    # Extract only business-relevant text
    text_content = soup.get_text(separator='\n', strip=True)
    
    # Filter for relevant keywords
    relevant_lines = []
    keywords = ['nav', 'expense', 'return', 'aum', 'exit load', 'minimum', 'investment', 
                'objective', 'risk', 'fund manager', 'benchmark', 'allocation', 'holdings']
    
    for line in text_content.split('\n'):
        line = line.strip()
        if line and len(line) > 10 and any(kw in line.lower() for kw in keywords):
    ...
    relevant_data = {
        ...
        'content': '\n'.join(relevant_lines[:100])  # Limit to 100 most relevant lines
    }
